...
struct Trie_t
{
  struct Trie_t *child[255];
  bool flag;
};

struct Trie_t *getnode()
{
  struct Trie_t *next_node = new Trie_t();
   for(int i=0;i<255;i++)
   {
    next_node->child[i]=NULL;
   }
   next_node->flag=false;
   return next_node;
}

void iinsert_in_Trie(struct Trie_t *root,string str)
{
  int ind;
 // cout<<"str---1"<<str<<endl;
  struct Trie_t *start=root;
  for(int i=0;i<str.length();i++)
  {  ind=str[i];
   // cout<<"str---"<<str[i]<<endl;
    if(start->child[ind]==NULL)
    start->child[ind]=getnode();
...
string search(struct Trie_t *root,string str)
{ //cout<<"str---"<<str<<endl;
  int ind;
  string final_word=str;
  struct Trie_t *start=root;
 // printf("%d",str[1]);
  for(int i=0;i<str.length();i++)
  {
...
int pos;
  while(start->flag!=true)
  {
    ...
    for(int i=0;i<255;i++)
    {  
      if(start->child[i]!=NULL&&f==0)
      {f=1;final_word+=char(i);pos=i;}

      else if(start->child[i]!=NULL&&f==1)
              return "";
      
    }
    start=start->child[pos];
  }
 //cout<<"final---"<<final_word<<endl;
 if(start->flag!=true)
  return "";
  else return final_word;
...
struct Trie_t* make_direct()
{

  DIR *d;
  struct dirent *dir;
  char crr[100];
  getcwd(crr,sizeof(crr));
  //cout<<"\n"<<crr<<endl;
  d=opendir(crr);
  //cout<<"stupid";
  char arr[100];
  int i=0;
  struct Trie_t *root=getnode();
  if(d!=NULL)
  {
    while((dir=readdir(d))!=NULL)
    {
      string rohan=dir->d_name;
      //cout<<"rohan---"<<rohan<<endl;
      iinsert_in_Trie(root,rohan);
...
struct Trie_t* make_rev_his()
{
  ...
  if(fp)
  {char ar[100];
    while(fgets(ar,sizeof(ar),fp))
    {  char *m=strtok(ar,DELIM);
           char ae[100];
           strcpy(ae,m);
           string hg=ae;
      string str=ae;

      iinsert_in_Trie(root,str);
    }
  }
  return root;
...
struct Trie_t *root=make_direct();
struct Trie_t *root1=make_rev_his(); 
    string s="";
  //string st=search(root,"nh");
  //cout<<"s--"<<st<<endl;
  while((a=getchar())!='\n')
  {
     if(a==127)
     ...
     // cout<<"\r"<<yy;
       ...
       op=0; 
        string ff=s.substr(0,2);
        string ss=s.substr(op+2,s.length());
        cout<<" "<<ss<<endl;
       ...
       string wr=ss.substr(op-1,ss.length());
       cout<<"wr--"<<wr<<endl;*/
       //cout<<"r--"<<root->child[int('n')]<<endl;
       string wr=s.substr(3,s.length());
      string ws=search(root,wr);
       cout<<"ws--"<<ws<<endl;
       s="cd "+ws;
       printf("\r                                                                  "); 
      string pp=print_prompt();
      pp+=s;
       printf("%s",pp.c_str());
      continue;
       }
